...
public:
	...
	auto submitTask(Func&& func, Args&&... args) -> std::future<decltype(func(args...))>
	{
		...
		// cached模式，任务处理比较紧急，适用场景:小而快的任务，需要根据任务数量和空闲线程的数量，判断是否需要创建新的线程出来
		if (poolMode_ == PoolMode::MODE_CACHED
					...
					if (poolMode_ == PoolMode::MODE_CACHED)
					{
							...
							notEmpty_.wait_for(lock, std::chrono::seconds(1)))
						{
								...
								&& curThreadSize_ > initThreadSize_)
							{
								// 开始回收当前线程
								// 记录线程数量的相关变量的值修改
								// 把线程对象从线程列表容器中删除，没有办法匹配threadFunc对应的是哪个thread对象
								// threadid => thread对象 => 删除
								threads_.erase(threadid); // 不是std::this_thread::get_id()
								curThreadSize_--;
								idleThreadSize_--;
...
private:
	...
	std::mutex taskQueMtx_; // 保护任务队列的线程安全
